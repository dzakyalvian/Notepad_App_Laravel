<div 
    wire:click="openEdit({{ $note->id }})"
    class="bg-keep-card dark:bg-[#202124] border border-keep-border dark:border-[#5f6368] rounded-lg p-4 flex flex-col gap-3 min-h-[160px] cursor-pointer transition-all hover:shadow-[0_1px_3px_rgba(60,64,67,0.15),0_1px_2px_rgba(60,64,67,0.3)] dark:hover:shadow-[0_1px_3px_rgba(0,0,0,0.5),0_1px_2px_rgba(0,0,0,0.3)] hover:border-keep-borderHover dark:hover:border-[#e8eaed] group">

    <div class="flex justify-between items-start gap-2">
        <p class="text-sm font-semibold text-keep-textPrimary dark:text-[#e8eaed] m-0 leading-snug">
            {{ $note->title }}
        </p>

        @if ($activeTab !== 'trash')
            <button 
                wire:click.stop="toggleFavorite({{ $note->id }})"
    ...
    <p class="text-xs text-keep-textBody dark:text-[#e8eaed] leading-relaxed m-0 overflow-hidden flex-1">
        {{ Str::limit(trim(strip_tags(str_replace(['<br>', '<br/>', '<br />', '<div>', '</p>', '<h1>', '<h2>'], ' ', $note->body))), 120) }}
    </p>
    ...
    <div class="flex justify-between items-center mt-auto pt-2">
        ...
        <div class="flex items-center gap-1">
            ...
            @if ($note->is_archived && $activeTab !== 'archive')
                <span class="text-[10px] px-2.5 py-0.5 rounded-full font-medium bg-gray-100 dark:bg-[#3c4043] text-gray-500 dark:text-[#9aa0a6]"
        ...
        @if ($activeTab === 'trash')
            ...
            <div class="flex gap-1.5">
                ...
                <button 
                    wire:click.stop="restore({{ $note->id }})"
                    class="text-[10px] py-1 px-2.5 rounded bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800 cursor-pointer font-medium transition-colors hover:bg-green-100 dark:hover:bg-green-900/50">
                    Restore
                </button>

                <button 
                    wire:click.stop="forceDelete({{ $note->id }})"
                    wire:confirm="Delete permanently?"
                    class="text-[10px] py-1 px-2.5 rounded bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 cursor-pointer font-medium transition-colors hover:bg-red-100 dark:hover:bg-red-900/50">
                    Delete
                </button>
        ...
        @else
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {{-- ARCHIVE --}}
                <button 
                    wire:click.stop="toggleArchive({{ $note->id }})" 
                ...
                {{-- DUPLICATE --}}
                <button 
                    wire:click.stop="duplicateNote({{ $note->id }})" 
                    class="bg-transparent border-none cursor-pointer text-keep-textSecondary dark:text-[#9aa0a6] text-xs p-1 rounded-full flex items-center justify-center w-7 h-7 transition-colors hover:bg-keep-bg dark:hover:bg-[#3c4043] hover:text-keep-textPrimary dark:hover:text-[#e8eaed]"
    ...
    <p class="text-[9px] text-keep-textSecondary/80 dark:text-[#9aa0a6] m-0 text-right">
